...
public class scr_grate : scr_grate_abstract
{
    ...
    private bool CheckIfPlayerIsCloserToPoint1()
    {
        ...
        float exit1Distance = Mathf.Sqrt(Mathf.Pow((playerPosition - exit1.position).x,2f) + 
            Mathf.Pow((playerPosition - exit1.position).y,2f));
        float exit2Distance = Mathf.Sqrt(Mathf.Pow((playerPosition - exit2.position).x, 2f) + 
            Mathf.Pow((playerPosition - exit2.position).y, 2f));
